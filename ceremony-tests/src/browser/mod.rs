...
pub struct Session {
    browser: Browser,
    /// The profile Chrome runs in, gone with the session.
...
impl Session {
        ...
        let (profile_dir, temp) = match platform.profile() {
            ...
            None => {
                let temp = tempfile::Builder::new()
                    .prefix("libid-ceremony-")
                    .tempdir()
                    .expect("a temporary profile directory");
    ...
    pub fn close(mut self) -> tokio::task::JoinHandle<()> {
        tokio::spawn(async move {
            ...
            // Chrome exits after the close returns, and its helpers a moment
            // later; the profile directory goes once nothing holds it.
            let _ = self.browser.wait().await;
            self.driving.abort();
            let Some(temp) = &self._profile else {
                return;
            };
            let profile = temp.path().to_path_buf();
            let mut removed = Ok(());
            for _ in 0..25 {
                removed = std::fs::remove_dir_all(&profile);
                if removed.is_ok() {
                    break;
                }
            ...
            if let Err(e) = removed {
                eprintln!("the profile {} stays behind: {e}", profile.display());
            }
        })
    }
}

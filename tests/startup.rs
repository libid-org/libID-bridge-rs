...
fn the_binary_serves_every_route_until_interrupted() {
    ...
    assert_eq!(record["platforms"]["github"]["clientId"], CLIENT_ID);
    assert_eq!(
    ...
    // The bridge binds before its first retrieval ends; the callback answers
    // `503` until that retrieval publishes a document.
    let callback = std::iter::repeat_with(|| {
        let reply = get("/auth/callback?code=abc&state=v1.9e1f", &[]);
        if reply.status == 503 {
    ...
    .take(500)
    .find(|reply| reply.status != 503)
    .expect("the first retrieval publishes within ten seconds");
...
fn a_configuration_the_binary_cannot_serve_stops_it() {
    let output = common::bridge::attempt(&config(""));
...
fn a_run_with_no_configuration_file_names_it() {
    let output = common::bridge::attempt_unconfigured();
    assert!(!output.status.success(), "{}", output.status);
    let stderr = String::from_utf8_lossy(&output.stderr);

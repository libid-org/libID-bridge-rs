...
impl Notary {
    ...
    pub async fn attesting() -> Notary {
        ...
        let accepting = crate::runtime().spawn(async move {
            ...
            while let Ok((socket, _)) = listener.accept().await {
                ...
                tokio::spawn(async move {
                    ...
                    // The prover sees only that no record came; the cause
                    // is the notary's, and is reported here.
                    if let Err(e) = serve(socket, sign, &recorded).await {
                        eprintln!("fixture notary: the session failed: {e}");
                    }
                });
            }
        });
